---
title: BM25 vs Dense 임베딩 기반 검색과 RRF 결합 성능 비교
layout: default
parent: RagRetrieval
nav_order: 3
permalink: /ragretrieval/rrf
# nav_exclude: true
# search_exclude: true
--- 
...
```
macOS에서는 eunjeon Java 라이브러리를 직접 설치하지 않고도 konlpy에서 Mecab이나 Komoran을 대체로 사용하는 경우가 많습니다.
하지만 eunjeon 기반인 Okt, Kkma, Hannanum, Komoran 등은 JVM을 통해 작동합니다.
...
# 문서 토큰화
tokenized_docs = [tokenize(content) for content in documents_df['content']]
bm25 = BM25Okapi(tokenized_docs)
