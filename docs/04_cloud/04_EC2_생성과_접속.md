...
##### Windows-방법 C: PuTTY (전통적 GUI 클라이언트)

OpenSSH 사용이 어렵거나 회사 환경에서 PuTTY가 표준이라면:

**Step 1. PuTTY 설치**

[putty.org](https://www.putty.org) → "MSI installer (64-bit)" 다운로드 → 설치. PuTTY와 PuTTYgen이 함께 설치됩니다.

**Step 2. .pem → .ppk 변환** ⚠️ PuTTY 핵심 단계

PuTTY는 OpenSSH 형식(.pem) 을 직접 읽지 못합니다. **PuTTYgen** 으로 변환해야 합니다.

1. 시작 메뉴에서 **PuTTYgen** 실행
2. 상단 메뉴 [Conversions → Import key] 클릭
3. 다운로드한 `my-key.pem` 선택
4. [Save private key] 클릭 → "without passphrase" 경고는 [Yes]
5. `my-key.ppk` 로 저장

**Step 3. PuTTY로 접속**

1. **PuTTY** 실행
2. 좌측 트리에서 [Connection → SSH → Auth → Credentials] 진입
3. "Private key file for authentication" 우측 [Browse] → `my-key.ppk` 선택
4. 좌측 트리 최상단 [Session] 으로 돌아가서:
   - **Host Name**: `ubuntu@<인스턴스의-퍼블릭-IP>`
   - **Port**: `22`
   - **Connection type**: SSH
5. 하단 "Saved Sessions" 칸에 이름(예: `aws-prod`) 입력 → [Save]
6. [Open] 클릭 → 첫 접속 시 호스트 키 확인 창 → [Accept]

저장된 세션은 다음번부터 더블클릭 한 번에 접속됩니다.
...
> ⚠️ **(PuTTY) "PuTTY Key File Format" 에러가 나면:**
>
> PuTTY는 `.pem` 을 직접 못 읽습니다. PuTTYgen으로 `.ppk` 로 변환했는지 확인하세요 (**Windows-방법 C Step 2**).
