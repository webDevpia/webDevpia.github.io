...
flowchart LR
    ...
    On -.->|"클라우드 전환"| Cloud
    style On fill:#fee2e2,stroke:#991b1b
    style Cloud fill:#dbeafe,stroke:#1e40af
```
...
**02장 AWS 가입 따라하기**(사전 과제) — 수업 전날까지 가입을 마친 상태로 참여합니다. 미완료자는 수업 시작 시 별도 안내합니다.

**03장 AWS 콘솔과 비용 관리**(수업 시작)에서는:
- AWS 콘솔을 둘러보고 **서울 리전**을 선택합니다
- **Zero-Spend Budget**과 Free Tier 알림을 설정해 예상치 못한 과금을 빠르게 감지합니다
- 루트 계정과 IAM 사용자의 차이를 가볍게 정리합니다
