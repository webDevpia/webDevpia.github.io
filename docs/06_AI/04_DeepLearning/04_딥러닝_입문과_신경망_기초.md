...
```mermaid
flowchart TB
    subgraph AI["🔵 인공지능 AI — 규칙 기반 시스템, 탐색, 추천"]
        subgraph ML["🟢 머신러닝 ML — 회귀, 결정트리, SVM"]
            subgraph DL["🟠 딥러닝 DL — CNN, RNN, Transformer"]
                core["다층 신경망으로\n자동 특징 학습"]
            end
        end
    end
    style AI fill:#DBEAFE,stroke:#1D4ED8,stroke-width:2px,color:#1E3A5F
    style ML fill:#D1FAE5,stroke:#059669,stroke-width:2px,color:#065F46
    style DL fill:#FEF3C7,stroke:#D97706,stroke-width:2px,color:#92400E
    style core fill:#FEF3C7,stroke:none,color:#92400E
```
...
def preprocess_user_image(image_path):
    """
    사용자가 작성한 숫자 이미지를 MNIST 형식처럼 변환
    반환 shape: (1, 1, 28, 28)
    """
    ...
    # 색 반전 여부 자동 보정
    # MNIST는 보통 "검은 배경 + 흰 숫자"에 가까운 형태
    # 사용자가 흰 배경에 검은 숫자를 썼다면 반전 필요
    img_np = np.array(img)

    # 평균 밝기가 밝으면(흰 배경 비중 큼) 반전
    if img_np.mean() > 127:
        img = ImageOps.invert(img)

    # 텐서 변환
    transform = transforms.ToTensor()
    img_tensor = transform(img)  # (1, 28, 28), 0~1

    # 배치 차원 추가 -> (1, 1, 28, 28)
    img_tensor = img_tensor.unsqueeze(0)

    return img_tensor, original_img, img


def predict_user_image(model, image_path, device):
    model.eval()

    img_tensor, original_img, processed_img = preprocess_user_image(image_path)
...
```
```
predict_user_image(model, "2.png", "cpu")
```
##### 흔한 실수 시나리오
...
→ **다음 장**: [5. 모델 성능 평가](/deeplearning/evaluation)
