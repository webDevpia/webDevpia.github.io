...
2. 콜백 함수 — 이미 쓰고 있었다!
3. Promise — "약속" 패턴
4. async/await — 더 깔끔한 방법
5. fetch로 데이터 가져오기 — JSONPlaceholder API
6. 실습 과제 — 기본 / 도전
7. 정리 — 핵심 요약 및 다음 장
...
```javascript
// btn은 document.querySelector('#btn')으로 선택한 버튼 요소입니다
...
// 함수 실행
getUserName_promise(1).then((name) => console.log(name));
getUserName_async(1).then((name) => console.log(name));
// 출력: Leanne Graham (둘 다 동일한 결과)
```
...
fetchData('https://jsonplaceholder.typicode.com/users/1')
  .then((data) => console.log(data.name)); // 출력: Leanne Graham

// 잘못된 URL — 에러 처리
fetchData('https://jsonplaceholder.typicode.com/users/9999')
  .then((data) => console.log(data)); // 출력: null (에러가 catch에서 처리됨)
```
...
아래 코드를 `index.html` 파일로 저장하고 브라우저에서 열어보세요. "사용자 불러오기" 버튼을 클릭하면 실제 API에서 데이터를 가져와 화면에 표시합니다.
...
safeFetch('https://jsonplaceholder.typicode.com/users/9999')
  .then((data) => console.log(data)); // 콘솔: "데이터 요청 실패: HTTP 404: Not Found" → 출력: null

// 잘못된 도메인 — 네트워크 에러
safeFetch('https://없는주소.example.com/api')
  .then((data) => console.log(data)); // 콘솔: "네트워크 연결을 확인해주세요" → 출력: null
```

> **참고**: `safeFetch`는 `fetch`를 감싸는 래퍼 함수입니다. 반복되는 에러 처리 패턴을 하나로 모아두면 코드가 간결해집니다. 위 세 가지 호출로 정상/HTTP 에러/네트워크 에러가 각각 어떻게 처리되는지 확인하세요.
