...
4. [주요 기능](#part4)
5. [에이전트 구성](#part5)
6. [Streamlit UI 설계](#part-ui)
7. [테스트 전략](#part-test)
8. [확장 아이디어](#part-extend)
9. [에이전트 함수 설명서](#part6)
...
이 프로젝트의 Streamlit UI는 크게 **채팅 영역**과 **사이드바**로 나뉩니다.

#### 메인 채팅 화면

- `st.chat_message` + `st.chat_input` 조합으로 대화형 인터페이스를 구성합니다.
- 사용자가 "배고파", "심심해"를 입력하면 챗봇이 LangGraph를 통해 추천 결과를 반환합니다.
- 추천 결과는 카드 형태(`st.container` + `st.columns`)로 예쁘게 표시됩니다.

#### 사이드바 (실시간 상황 표시)

- **현재 시간대**: 아침 / 점심 / 저녁 / 야간 중 하나를 표시
- **계절**: 봄 / 여름 / 가을 / 겨울 (현재 월 기준)
- **날씨**: OpenWeather API에서 받아온 날씨 상태 (맑음, 흐림, 비 등)
- 매 요청마다 자동으로 갱신되어 사용자에게 현재 상황을 직관적으로 안내합니다.

#### 추천 결과 카드

- 음식 또는 활동 추천 2개를 나란히 카드로 표시합니다.
- 각 카드에는 추천 이름, 간단한 설명, 관련 이모지가 포함됩니다.

#### 장소 검색 결과

- Kakao 장소 검색 결과를 표로 출력합니다.
- 장소명, 주소, 카카오맵 링크(클릭 가능)를 함께 표시합니다.
...
> 💡 Streamlit의 `st.columns(2)`를 활용하면 추천 카드 2개를 좌우로 배치할 수 있습니다.
...
각 에이전트를 개별적으로 검증한 후 전체 흐름을 통합 테스트합니다.  
테스트는 `pytest`를 기반으로 하며, 외부 API 의존 에이전트는 **모킹(mocking)** 처리합니다.

| 에이전트 | 테스트 유형 | 검증 내용 |
|---------|-----------|----------|
| `classify_intent` | 단위 테스트 | `food` / `activity` / `unknown` 분류 정확성 |
| `get_time_slot` | 단위 테스트 | 시간대별 올바른 슬롯 반환 (아침/점심/저녁/야간) |
| `get_weather` | 통합 테스트 | API 호출 성공 + 에러 시 기본값(`"Clear"`) 처리 |
| `recommend_food` | 모킹 테스트 | LLM 응답이 리스트 2개 형식인지 검증 |
| 전체 그래프 | E2E 테스트 | 입력 → 최종 응답(`final_message`) 전체 흐름 |
...
# 특정 에이전트만 테스트
pytest test_graph.py::test_classify_intent -v

# 터미널 직접 실행 테스트
python test_runner.py
```

> ⚠️ 외부 API(`get_weather`, `search_place`)는 실제 API 키가 없는 환경에서는 반드시 모킹 처리해야 합니다.  
> `unittest.mock.patch`를 활용하여 API 응답을 미리 정의된 값으로 대체하세요.
...
→ **다음 장**: [16. 프로젝트 구현](/llm/langgraph/agent_task)
