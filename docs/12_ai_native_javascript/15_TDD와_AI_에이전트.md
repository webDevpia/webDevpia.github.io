...
> 11장(ToDo 앱)에서는 요구사항을 쓰면 **AI가 테스트를 생성**하고, 학생이 검토했습니다.
> 이번 장에서는 학생이 **테스트를 직접 작성**합니다. 왜 다를까요?

| | 11장 (ToDo 프로젝트) | 14장 (TDD) |
|--|---------------------|-----------|
| 테스트 작성 | AI가 생성 → 학생이 검토 | **학생이 직접 작성** |
| 목적 | AI 코드 검증 능력 | **설계 능력** |
| 핵심 질문 | "이 테스트가 맞는가?" | "어떤 테스트를 써야 하는가?" |

TDD에서 테스트를 작성하는 것은 **코딩이 아니라 설계**입니다. "이 함수가 어떻게 동작해야 하는가?"를 코드로 정의하는 행위입니다. 11장의 `requirements.md`가 자연어로 설계했다면, TDD의 테스트 코드는 **프로그래밍 언어로 설계**하는 것입니다.

> 💡 **정리**: requirements.md = 자연어 설계, 테스트 코드 = 코드로 된 설계. 둘 다 "무엇을 만들지"를 정의하는 방법입니다.
