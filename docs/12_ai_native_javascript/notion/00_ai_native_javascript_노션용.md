...
기존의 코딩 교육은 문법을 외우고, 혼자 코드를 작성하는 방식이었습니다.이 과정은 다릅니다. **AI를 학습 파트너로 활용**하되, 기초 역량은 반드시 직접 만들어갑니다.
...
- 강사 시연을 보고 따라하며, 미니 프로젝트를 완성합니다
- 학습 속도와 이해도에 따라 유연하게 진행합니다
...
flowchart LR
    subgraph P0["Phase 0 — 워밍업"]
        A["프로그래밍이 뭔지 알고<br/>첫 웹페이지를 만든다"]
    end
    subgraph P1["Phase 1 — 기초 + AI 설명자"]
        B["코드는 내가 쓰고<br/>모르는 건 AI에게 물어본다"]
    end
    subgraph P2["Phase 2 — 검증자 + AI 리뷰어"]
        C["AI가 쓴 코드를<br/>내가 읽고, 검증하고, 고친다"]
    end
    subgraph P3["Phase 3 — AI-Native 개발"]
        D["내가 설계하고<br/>AI가 구현하고, 내가 검증한다"]
    end
    P0 --> P1 --> P2 --> P3
...
> 💡 이 과정에서 CSS/Tailwind를 별도로 가르치지 않습니다. 이전 과정(HTML/CSS/Tailwind)에서 배운 지식을 활용하며, 프로젝트의 스타일링은 Tailwind CDN + AI 생성을 조합합니다. 여러분은 JavaScript 로직과 테스트에 집중합니다.
...
| 14 | [Custom Agent](/ai-native-js/agent)                         | AI에게 전문가 역할 부여             |
| 15 | [TDD](/ai-native-js/tdd)                                    | 요구사항 → 테스트 → AI 구현       |
| 16 | [통합 프로젝트 — 날씨 앱](/ai-native-js/weather-app)         | 전체 AI-Native 파이프라인           |
| 17 | [통합 프로젝트 — 영화 검색 앱](/ai-native-js/movie-app)      | 독립 AI-Native 실행 (선택형 캡스톤) |

## 최종 산출물

이 과정을 마치면 세 가지가 완성됩니다:
