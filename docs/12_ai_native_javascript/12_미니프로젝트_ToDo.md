...
`index.html` 파일을 만들고 아래 내용을 복사하세요. 각 요소의 `id`와 `class`가 어떤 역할인지 읽어보세요.
...
> **테스트를 먼저 준비하고, AI에게 구현을 요청한다.**
>
> 테스트는 "무엇이 되어야 하는지"의 명세입니다.
> AI가 코드를 만들었을 때 테스트가 통과하면 올바른 구현입니다.
> 테스트가 실패하면 AI의 코드에 문제가 있다는 신호입니다.

### Step 1: 테스트 코드 읽고 이해하기

아래 테스트 코드를 `tests/todo.test.js`에 복사하세요. 복사한 뒤 **각 테스트가 무엇을 검증하는지** 읽어보세요.
...
**테스트 읽기 가이드** — 각 테스트가 무엇을 확인하는지 정리해보세요:

| 테스트 | 검증 내용 |
|--------|----------|
| `addTodo` 첫 번째 | 텍스트를 넣으면 배열에 새 할 일이 추가되는가? |
| `addTodo` 두 번째 | 빈 문자열은 추가되지 않는가? |
| `addTodo` 네 번째 | 원본 배열이 변경되지 않는가? (불변성) |
| `toggleTodo` | 완료/미완료 상태가 반전되는가? |
| `deleteTodo` | 해당 id가 제거되는가? |
| `filterTodos` | 전체/완료/미완료 필터가 동작하는가? |

> 💡 이 테스트들이 곧 **요구사항의 명세**입니다. AI에게 "이 테스트를 통과하는 코드를 만들어줘"라고 요청하면, 테스트가 곧 지시서 역할을 합니다.
