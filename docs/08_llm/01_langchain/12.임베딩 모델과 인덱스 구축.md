...
1. [임베딩(Embedding)이란?](#임베딩embedding이란) - 텍스트를 벡터로 변환하는 개념
2. [Pinecone 인덱스란?](#pinecone-인덱스란) - 벡터 저장 구조와 핵심 역할
3. [Pinecone 환경 설정](#pinecone-환경-설정) - API 키 및 .env 파일 구성
4. [Pinecone 인덱스 생성](#pinecone-인덱스-생성) - 서버리스 인덱스 초기화 코드
5. [한국 영화 메타데이터 구성](#한국-영화-메타데이터-구성) - 실습 데이터 필드 설계
6. [하이브리드 검색을 고려한 필드 설계](#하이브리드-검색을-고려한-필드-설계) - 메타데이터 필터 조건 설계
7. [임베딩 생성 및 Pinecone 업서트](#임베딩-생성-및-pinecone-업서트) - 문서 벡터화 및 저장 실습
8. [정리](#정리) - 전체 흐름 요약
