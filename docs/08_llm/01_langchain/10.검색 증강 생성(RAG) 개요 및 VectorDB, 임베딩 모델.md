...
### 📦 환경 준비
```bash
pip install -U pinecone-client langchain-pinecone pydantic
```
...
if not pc.has_index("example-index"):
    pc.create_index(name="example-index", dimension=1536)
...
```
Query 1 Results:
* [SIM=0.581738] LangChain is great for building AI apps. [{'source': 'tweet'}]
* [SIM=0.581738] Building an exciting new project with LangChain - come check it out! [{'source': 'tweet'}]

Query 2 Results:
* [SIM=0.581738] Tomorrow will be cloudy with a high of 62 degrees. [{'source': 'news'}]
