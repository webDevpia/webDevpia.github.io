...
1. [RAG란 무엇인가?](#rag란-무엇인가) - RAG 개념과 기존 LLM 한계 보완
2. [작동 구조 요약](#작동-구조-요약) - 임베딩 → VectorDB → 검색 → 생성 흐름
3. [VectorDB란?](#vectordb란) - 대표 벡터 데이터베이스와 주요 기능
4. [임베딩(Embedding) 모델](#임베딩embedding-모델) - 텍스트 벡터 변환과 주요 모델 비교
5. [RAG 실습: Pinecone + LangChain](#rag-실습-pinecone--langchain) - 환경 준비, 문서 저장, 검색 코드
6. [정리](#정리) - 핵심 구조 및 도구 요약
