...
1. [LCEL이란?](#1-lcel이란) - 이전 챕터와의 연결, 공장 비유
2. [실행 방식 3가지](#2-실행-방식-3가지-invoke-stream-batch) - invoke, stream, batch
3. [RunnableParallel — 동시에 여러 작업](#3-runnableparallel--동시에-여러-작업) - 요약 + 키워드를 한 번에
4. [RunnableLambda — 내 함수 끼워넣기](#4-runnablelambda--내-함수-끼워넣기) - LLM 결과 후처리
5. [RunnableBranch — 조건에 따라 분기](#5-runnablebranch--조건에-따라-분기) - 입력에 따라 다른 체인 실행
6. [실전 예제 — 요약 + 키워드 동시 추출](#6-실전-예제--요약--키워드-동시-추출)
7. [정리](#7-정리)
...
## 2. 실행 방식 3가지: invoke, stream, batch
