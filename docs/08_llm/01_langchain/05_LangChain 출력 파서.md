...
1. [출력 파서(Output Parser)란?](#1-출력-파서output-parser란) - OutputParser의 필요성과 개념
2. [기본 사용법 — StrOutputParser](#2-기본-사용법--stroutputparser) - 문자열 출력 처리
3. [JSON 형식 강제 — JsonOutputParser](#3-json-형식-강제--jsonoutputparser) - 구조화된 JSON 출력
4. [정적 타입 검증 — with_structured_output()](#4-정적-타입-검증--with_structured_output) - Pydantic 스키마 기반 출력 보장
5. [실전 예시: JSON 결과를 DB로 저장하기](#5-실전-예시-json-결과를-db로-저장하기) - SQLite 연동 실습
6. [실습 포인트](#-실습-포인트) - 핵심 정리
