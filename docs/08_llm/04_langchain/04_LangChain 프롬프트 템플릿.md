...
1. [왜 프롬프트 템플릿인가?](#1-왜-프롬프트-템플릿인가) - 하드코딩 vs 템플릿 비교
2. [PromptTemplate — 기본 텍스트 템플릿](#2-prompttemplate--기본-텍스트-템플릿) - 변수 삽입과 format()
3. [ChatPromptTemplate — 역할 분리 템플릿](#3-chatprompttemplate--역할-분리-템플릿) - system/user 역할
4. [LLM과 연결해서 실행하기](#4-llm과-연결해서-실행하기) - 체인 구성과 invoke()
5. [partial() — 변수 미리 고정하기](#5-partial--변수-미리-고정하기) - 반복 값 설정
6. [여러 프롬프트를 연결한 체인](#6-여러-프롬프트를-연결한-체인) - 프롬프트 조합
7. [시나리오 기반 프롬프트 작성](#7-시나리오-기반-프롬프트-작성) - 이메일 작성 예시
8. [정리](#8-정리)
...
## 3. ChatPromptTemplate — 역할 분리 템플릿
