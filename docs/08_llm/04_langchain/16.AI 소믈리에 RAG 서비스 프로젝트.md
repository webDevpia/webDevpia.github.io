...
# 외부 URL의 이미지를 다운로드해서 base64 형식의 데이터 URL로 변환합니다
# OpenAI Vision API는 이미지를 직접 URL이 아닌 base64 인코딩된 문자열로도 받을 수 있습니다
def image_to_base64_url(image_url: str):
    # URL에서 이미지 파일을 다운로드합니다
    response = requests.get(image_url)
    response.raise_for_status()  # 이미지 다운로드 실패 시 에러 발생
    # 응답 헤더에서 파일 형식(MIME 타입)을 가져옵니다 (예: image/jpeg)
    mime_type = response.headers.get("Content-Type", "image/png")
    # 이미지 바이너리(0,1 데이터)를 base64 문자열로 변환합니다
...
# 외부 이미지 URL을 API 전송 가능한 base64 데이터 URL로 변환합니다
