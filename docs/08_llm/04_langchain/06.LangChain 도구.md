...
1. [도구란 무엇인가?](#1-도구란-무엇인가) - LLM과 도구의 관계 이해
2. [도구 기본 사용법](#2-도구-기본-사용법) - @tool 데코레이터, Pydantic 스키마, 스트리밍
3. [외부 API 호출 Tool 구현하기](#3-외부-api-호출-tool-구현하기-예-날씨-api-뉴스-api) - 날씨 API, 뉴스 API 실습
...
{: .warning }
> **여기부터 심화 내용입니다.** 2.4~2.7과 3장은 LangGraph를 활용한 고급 기능입니다. 기본 도구 사용(2.1~2.3)만으로도 충분히 실습 가능하며, 심화 내용은 필요할 때 돌아와서 학습해도 됩니다.
>
> **버전 참고:** 2.4~2.7의 `create_agent`, `ToolRuntime` API는 **LangChain v1.0 이상**에서 동작합니다. 현재 실습 환경(v0.3.x)에서는 `ImportError`가 발생할 수 있습니다. v0.3.x에서는 `langgraph.prebuilt.create_react_agent`를 사용하세요.
...
memory = MemorySaver()  # LangGraph는 상태 기반이므로 메모리 저장소 필요
...
- **중급**: Pydantic `args_schema`를 사용하여 도시명과 단위(섭씨/화씨)를 입력받는 날씨 도구를 만들어 보세요
- **심화**: 날씨 도구와 뉴스 도구를 결합한 에이전트를 만들어, "서울 날씨와 AI 뉴스 알려줘"에 응답하게 해 보세요
