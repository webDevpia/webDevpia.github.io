---
title: 4. LangGraph 챗봇에서 도구 사용하기
layout: default
grand_parent: LLM
parent: LangGraph
nav_order: 4
permalink: /llm/langgraph/chat_tool
# nav_exclude: true
# search_exclude: true
--- 
...
[Tavily(타빌리)](https://tavily.com/)

가입시 default로 기본 Api key가 있음.   
(default key 사용, 재발급시 사용하기까지 일정 시간지나야 사용 가능.). 

![](./img/langgraph/langgraph001.png)
...
[
    ...
    },
    {
        ...
        'content': 'LangGraph provides node and edge abstractions...'
    }
]
```

- `TavilySearch`: 사용자가 입력한 질의에 대해 최대 2개의 결과를 가져오는 검색 도구를 만들었습니다.
...
class ToolNode:
    ...
    def __call__(self, inputs):
        if messages := inputs.get("messages", []): # Python 3.8 이상에서 사용 가능한 할당 표현식 “해당 값이 있으면 변수에 할당”
            message = messages[-1]
        else:
...
- `workflow.add_edge("tools", "chatbot")`은 "tools" 노드에서 다시 "chatbot" 노드로 연결하는 엣지를 정의합니다. 이 엣지는 도구 실행 후 결과를 챗봇 노드로 반환하는 역할을 합니다.
...
아래는 도구를 사용하는 챗봇의 그래프 구조입니다:

```mermaid
graph TD
    __start__([__start__]) --> chatbot[chatbot]
    chatbot -.->|도구 호출 필요| tools[tools]
    chatbot -.->|도구 불필요| __end__([__end__])
    tools --> chatbot
```
...
**실행 결과 (예시):**
```
LangGraph에서 '노드(Node)'란 하나의 에이전트(Agent) 또는 처리 단계를 의미합니다.
각 노드는 자신만의 LLM, 프롬프트, 도구를 가질 수 있으며...
```

도구 호출 과정을 상세히 확인하려면 다음과 같이 메시지를 순회합니다:
...
**실행 결과 (예시):**
```
👤 사용자: LangGraph에서 '노드'란 무엇인가요?
🤖 AI:
  🔧 도구 호출: tavily_search({'query': "LangGraph 노드란 무엇인가"})
  📋 도구 결과: [{"url": "https://langchain-ai.github.io/langgraph/", "content": "LangGraph is a li...
🤖 AI: LangGraph에서 '노드(Node)'란 하나의 에이전트(Agent) 또는 처리 단계를 의미합니다...
```

> 💡 **LLM이 도구를 사용하지 않는 경우:** "안녕하세요"처럼 검색이 불필요한 질문에는 LLM이 직접 응답하며, 도구 호출 없이 바로 `END`로 이동합니다.
...
- `workflow.add_node("chatbot", chatbot)`은 그래프에 "chatbot"이라는 이름의 노드를 추가하며, 이 노드는 앞에서 정의한 `chatbot` 함수를 실행합니다.
- `workflow.add_node("tools", tool_node)`는 "tools"이라는 이름의 노드를 추가하며, 이 노드는 도구 실행을 관리합니다.
- `workflow.add_conditional_edges("chatbot", tools_condition)`은 "chatbot" 노드에서 "tools" 노드로 연결되는 조건부 엣지를 정의합니다. 이 엣지는 챗봇이 도구를 사용할 필요가 있을 때만 활성화됩니다.
- `workflow.add_edge("tools", "chatbot")`은 "tools" 노드에서 다시 "chatbot" 노드로 연결하는 엣지를 정의합니다. 이 엣지는 도구 실행 후 결과를 챗봇 노드로 반환하는 역할을 합니다.
- `workflow.set_entry_point("chatbot")`은 그래프의 시작점을 "chatbot" 노드로 설정합니다. 즉, 챗봇이 처음 실행되는 지점입니다.
- `workflow.compile()`은 정의한 그래프를 실제로 실행 가능한 형태로 컴파일하여 최적화 상태를 준비합니다.
