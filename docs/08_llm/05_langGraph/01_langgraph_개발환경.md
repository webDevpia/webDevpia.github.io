...
```bash
uv add python-dotenv notebook langgraph langchain-openai langchain-community langchain-tavily langchain-ollama langgraph-checkpoint-sqlite streamlit aiohttp
```
...
## 5. 프로젝트 폴더 구조 가이드 [↑](#toc)

강의 전반에서 사용할 권장 폴더 구조입니다. 처음부터 이 구조를 잡아두면 코드가 늘어나도 정리가 쉽습니다.
...
| 경로 | 역할 |
|------|------|
| `.env` | OpenAI, Tavily 등 API 키를 저장합니다. Git에 절대 올리지 않도록 `.gitignore`에 추가하세요. |
| `pyproject.toml` | uv로 관리하는 의존성 목록입니다. `uv add` 명령 시 자동으로 업데이트됩니다. |
| `agents/` | 각 LLM 에이전트 함수를 모아두는 폴더입니다. 기능별로 파일을 분리하면 관리가 편합니다. |
| `tools/` | 웹 검색, 계산기 등 에이전트가 사용하는 도구(Tool) 함수를 보관합니다. |
| `graphs/` | `StateGraph`를 정의하고 컴파일하는 코드를 넣습니다. 실습별로 파일을 분리할 수 있습니다. |
| `config.py` | `python-dotenv`를 이용해 `.env`를 로딩하는 설정 파일입니다. 모든 모듈에서 임포트해서 씁니다. |
| `app.py` | 강의 후반부에 Streamlit으로 UI를 구성할 때 사용합니다. |

> 💡 **초반에는 단일 `.py` 파일로 실습해도 충분합니다.** 코드가 늘어나는 중반부터 위 구조를 적용하면 됩니다.
