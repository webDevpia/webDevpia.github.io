...
3. [챗봇 실행](#part3)
4. [스트리밍 출력](#part4)
   - [기본 스트리밍 (stream_mode='values')](#part4)
   - [토큰 단위 스트리밍 (stream_mode='messages')](#part4-messages)
5. [SystemMessage로 챗봇 성격 설정하기](#part4-system)
6. [대화형 챗봇 만들기](#part5)
...
<a id="part4-messages"></a>

### 토큰 단위 스트리밍 (stream_mode='messages')

`stream_mode="values"`(기본값)는 노드가 완료될 때마다 전체 상태를 반환합니다. 반면 `stream_mode="messages"`를 사용하면 LLM이 생성하는 토큰을 하나씩 받아볼 수 있어 사용자에게 더 빠른 응답감을 줄 수 있습니다.

```python
for msg, metadata in graph.stream(
    {"messages": [("user", "파이썬이란 무엇인가요?")]},
...
**실행 결과 (예시):**
```
파이썬(Python)은 1991년 귀도 반 로섬이 개발한 고수준 프로그래밍 언어입니다. 문법이 간결하고 읽기 쉬워...
```

#### stream_mode 비교

| stream_mode | 반환 단위 | 특징 | 주요 사용처 |
|---|---|---|---|
| `"values"` (기본값) | 노드 실행 완료 후 전체 상태 | 중간 상태를 단계별로 확인 가능 | 디버깅, 노드별 결과 확인 |
| `"updates"` | 노드 실행 완료 후 변경된 부분만 | 변경된 값만 반환하여 네트워크 효율적 | 상태 변화 추적 |
| `"messages"` | LLM이 생성하는 토큰 단위 | 글자가 하나씩 출력되는 효과 | 사용자 대면 UI, 실시간 출력 |

> 💡 스트리밍에 대해 더 알고 싶다면 8장 '스트리밍 심화'를 참고하세요.

<a id="part4-system"></a>

### SystemMessage로 챗봇 성격 설정하기

`SystemMessage`는 사용자 메시지 앞에 붙이는 지시문으로, LLM의 역할과 응답 방식을 설정합니다. 챗봇 노드 함수 안에서 `state["messages"]` 앞에 `SystemMessage`를 추가하면 됩니다.

```python
from langchain_core.messages import SystemMessage

def chatbot_with_persona(state: State):
    system = SystemMessage(content="당신은 친근한 한국어 튜터입니다. 모든 설명에 비유를 사용합니다.")
...
**SystemMessage 없이 실행:**
```python
response = graph.invoke({"messages": [("user", "변수란 무엇인가요?")]})
print(response["messages"][-1].content)
# 변수(Variable)는 데이터를 저장하는 메모리 공간입니다...
```

**SystemMessage 있는 챗봇으로 실행:**
```python
workflow2 = StateGraph(State)
workflow2.add_node("chatbot", chatbot_with_persona)
workflow2.add_edge(START, "chatbot")
workflow2.add_edge("chatbot", END)
graph2 = workflow2.compile()

response = graph2.invoke({"messages": [("user", "변수란 무엇인가요?")]})
print(response["messages"][-1].content)
# 변수는 마치 이름이 붙은 상자와 같아요! 상자 안에 숫자, 글자 등 무엇이든 넣을 수 있고,
# 나중에 상자 이름만 부르면 그 안에 있는 것을 꺼낼 수 있습니다...
```

> 💡 `SystemMessage`의 내용을 바꾸면 같은 질문에도 전혀 다른 스타일의 답변을 얻을 수 있습니다. 실습 미션에서 직접 바꿔보세요.
...
3. `stream_mode`를 `'messages'`로 변경하여 글자가 하나씩 출력되는 것을 확인해 보세요.

---

→ **다음 장**: [4. 챗봇에서 도구 사용하기](/llm/langgraph/chat_tool)
