...
이 Lab에서는 **LangGraph**, **OpenAI API**, 그리고 **Kakao 장소 검색 API**를 활용하여 간단한 장소 검색 챗봇을 만들어봅니다.

<a id="toc"></a>

## 진행 순서

1. [개발 환경 설정 및 API 키 준비](#part1)
2. [상태 및 데이터 구조 정의](#part2)
3. [LLM 모델 초기화하기](#part3)
4. [키워드 추출 도구 정의하기](#part4)
...
- **Kakao REST API 키**: Kakao 지도/로컬 API를 사용하기 위해 Kakao Developers에서 **REST API 키**를 발급받아야 합니다.  
  발급받은 키를 환경변수 `KAKAO_API_KEY`에 저장하거나 `.env` 파일에 `KAKAO_API_KEY=본인의_KAKAO_API키` 형태로 추가하세요.  
    ...
    ![](./img/langgraph/langgraph002.png)
      ...
      REST API키 복사해서 .env 파일에 추가
    ![](./img/langgraph/langgraph003.png)

      플렛폼 메뉴의 web 항목에 `Web 플랫폼 등록`클릭 
    ![](./img/langgraph/langgraph004.png)

    http://localhost:8080 입력, 
    ![](./img/langgraph/langgraph005.png)

    카카오맵 메뉴에 활성화 버튼 체크함.
    ![](./img/langgraph/langgraph006.png)
...
## 2단계: 상태(State) 및 데이터 구조 정의 [↑](#toc)
이 챗봇은 대화 상태를 유지하며 동작합니다.  
우선 **상태를 표현할 클래스**와 **장소 정보 구조**를 정의해보겠습니다:
...
## 6단계: LangGraph로 대화 흐름 그래프 만들기 [↑](#toc)
...
- `add_edge(START, "chatbot")`: 그래프의 시작 노드를 지정합니다. START에서 "chatbot"으로 연결하여, 맨 처음에 LLM이 사용자 질문을 처리하도록 합니다.
...
initial_state2: State = {
    "messages": [HumanMessage(content="서울에서 애견동반 가능한 카페 알려줘")],
    "search_query": None,
    "search_results": None
}
final_state = graph.invoke(initial_state2)
