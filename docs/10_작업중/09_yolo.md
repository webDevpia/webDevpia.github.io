...
3. [객체 탐지의 발전](#part4)
4. [YOLO 실습: Ultralytics](#part5)
5. [YOLO 실습: 웹캠 객체 탐지](#part6)
6. [더 알아보기: YOLO 너머의 객체 탐지](#part6b)
7. [통합 정리](#part7)
...
- 쉽게 말해, **"두 박스가 얼마나 겹치는가"의 비율**입니다 (0이면 전혀 안 겹침, 1이면 완전히 일치)
...
**X축에 재현율, Y축에 정밀도를 놓고 그린 그래프**입니다. 곡선이 오른쪽 위 모서리에 가까울수록 "꼼꼼하면서도 빠짐없는" 좋은 모델입니다.

> **비유**: CCTV로 도둑을 잡는 보안 시스템을 생각해 봅시다.
> - 시스템의 **민감도(= 신뢰도 기준값)**를 조절할 수 있습니다
> - 민감도를 **최대로 높이면**: 확실한 도둑만 경보 → 오경보 적음(정밀도↑), 하지만 놓치는 도둑 많음(재현율↓)
> - 민감도를 **최소로 낮추면**: 조금만 수상해도 경보 → 거의 다 잡음(재현율↑), 하지만 오경보 폭증(정밀도↓)
>
> **PR 곡선**은 이 민감도를 0에서 1까지 바꿔가며, 매 단계의 정밀도와 재현율을 점으로 찍어 연결한 것입니다.

##### PR 곡선이 만들어지는 과정

앞에서 배운 보행자 탐지를 다시 생각해 봅시다. YOLO가 이미지에서 여러 개의 바운딩 박스를 찾았고, 각각에 **신뢰도(confidence)** — "이것이 보행자일 확률"을 0~1 사이 숫자로 출력했습니다.

이때 **신뢰도 기준값**을 바꾸면 "어떤 박스를 보행자로 인정할지"가 달라지고, 정밀도와 재현율이 함께 변합니다.

| 신뢰도 기준값 | "보행자"로 인정하는 기준 | 정밀도 | 재현율 | 상황 |
|:---:|:---|:---:|:---:|:---|
| **0.9** (매우 까다로움) | 확률 90% 이상만 인정 | 0.95 | 0.20 | 거의 틀리지 않지만, 많이 놓침 |
| **0.7** | 확률 70% 이상 인정 | 0.88 | 0.55 | 조금 느슨하게, 더 많이 찾음 |
| **0.5** (기본값) | 확률 50% 이상 인정 | 0.75 | 0.80 | 균형 잡힌 상태 |
| **0.3** (느슨함) | 확률 30% 이상 인정 | 0.50 | 0.92 | 거의 다 찾지만 오탐도 많음 |
| **0.1** (매우 느슨함) | 확률 10% 이상 인정 | 0.30 | 0.98 | 거의 모든 것을 보행자로 판단 |

**이 표의 각 행을 그래프에 점으로 찍고 연결하면 → PR 곡선이 됩니다.**
...
PR 곡선이 **오른쪽 위 모서리에 가까울수록** 좋은 모델입니다. 

- **좋은 모델**: 재현율을 높여도 정밀도가 천천히 감소 → 곡선 아래 **면적(AP)이 넓음**
- **나쁜 모델**: 재현율을 조금만 높여도 정밀도가 급락 → 곡선 아래 **면적(AP)이 좁음**

![정밀도-재현율 곡선 예시](./img/yolo/object_detaction006.png)

#### AP (Average Precision)와 mAP (mean Average Precision)

**AP**는 위 PR 곡선의 **아래쪽 면적**입니다. 면적이 넓을수록(1에 가까울수록) 좋은 모델입니다. 0~1 사이의 값을 가집니다.

- **AP**: **하나의 클래스**(예: "자동차")에 대한 성능 점수
- **mAP**: **모든 클래스의 AP를 평균**낸 종합 점수

```mermaid
graph TD
    mAP["<b>mAP</b><br/>전체 평균 (종합 성적표)"]
    AP1["AP(자동차)<br/>과목별 성적"]
    AP2["AP(사람)<br/>과목별 성적"]
    AP3["AP(개)<br/>과목별 성적"]
    PR1["PR 곡선(자동차)<br/>시험 결과지"]
    PR2["PR 곡선(사람)<br/>시험 결과지"]
    PR3["PR 곡선(개)<br/>시험 결과지"]
...
> **비유**: 시험에 비유하면, PR 곡선은 "시험 결과지", AP는 "한 과목 점수", mAP는 "전 과목 평균"입니다. COCO의 mAP는 같은 시험을 10가지 난이도(IOU 0.5~0.95)로 보고 평균을 내는 것이어서, 더 엄격한 평가입니다.
...
> **용어 정리**
> - **Region Proposal(영역 제안)**: "이 부분에 객체가 있을 것 같다"고 후보 영역을 먼저 찾는 과정
> - **ROI Pooling**: 다양한 크기의 후보 영역을 같은 크기로 맞추는 기법
> - **RPN(Region Proposal Network)**: 영역 제안을 별도 알고리즘이 아닌 신경망이 직접 수행

### 1-Stage Detector: "빠르고 실시간"

| 모델 | 연도 | 핵심 아이디어 |
|------|------|-------------|
| **YOLOv1** | 2015 | 이미지를 그리드로 나눠 한 번에 탐지. 최초의 1-stage detector (Redmon et al., [arXiv:1506.02640](https://arxiv.org/abs/1506.02640)) |
| YOLOv2 | 2016 | Batch Norm, Anchor Box 도입으로 정확도 개선 (Redmon & Farhadi, [arXiv:1612.08242](https://arxiv.org/abs/1612.08242)) |
| **SSD** | 2016 | 다중 스케일 Feature Map으로 작은 객체도 탐지. 30~40 fps 달성 ([arXiv:1512.02325](https://arxiv.org/abs/1512.02325), ECCV 2016) |
| YOLOv3 | 2018 | FPN 도입으로 다중 스케일 탐지, 정확도 대폭 향상 (Redmon & Farhadi, [arXiv:1804.02767](https://arxiv.org/abs/1804.02767)) |
| RetinaNet | 2017 | Focal Loss로 배경/객체 불균형 해결. 1-stage이면서 2-stage급 정확도 달성 ([arXiv:1708.02002](https://arxiv.org/abs/1708.02002)) |

> **핵심 흐름**: 1-stage 방식은 **영역 제안 없이 이미지 전체를 한 번에 처리**하여 실시간 탐지가 가능해졌습니다.

> **용어 정리**
> - **Anchor Box**: 미리 정해둔 여러 크기/비율의 기본 탐지 틀. 모델이 이 틀을 기준으로 위치를 조정
> - **FPN(Feature Pyramid Network)**: 크고 작은 특징을 동시에 보는 구조. 작은 객체도 잘 탐지할 수 있게 함
> - **Focal Loss**: 쉬운 예제(배경)의 영향을 줄이고 어려운 예제(객체)에 집중하는 손실 함수
...
| YOLOv6 | 2022 | **Meituan(메이투안)** | **EfficientRep** 백본, Anchor-Free 구조 ([arXiv:2209.02976](https://arxiv.org/abs/2209.02976)) |
| YOLOv7 | 2022 | Wang, Bochkovskiy, Liao | E-ELAN, 속도-정확도 균형 SOTA ([arXiv:2207.02696](https://arxiv.org/abs/2207.02696)) |
| YOLOv8 | 2023 | Ultralytics | 모듈화, 탐지/분할/분류 통합 프레임워크 |
| YOLOv9 | 2024 | Wang & Liao | PGI, GELAN 도입 ([arXiv:2402.13616](https://arxiv.org/abs/2402.13616)) |
| YOLOv10 | 2024 | 칭화대 | NMS-free end-to-end ([arXiv:2405.14458](https://arxiv.org/abs/2405.14458)) |
...
> **참고**: YOLOv5 이후부터는 원저자(Redmon)가 아닌 다양한 팀이 독립적으로 발전시키고 있습니다. Ultralytics는 YOLOv5, v8, YOLO11, YOLO26을 개발했고, 이 실습에서 사용하는 프레임워크입니다. 프로덕션 워크로드에는 **YOLO26 또는 YOLO11**이 권장됩니다.
...
> **참고**: 아래 `capture_image()` 함수의 JavaScript 코드는 이해하지 않아도 됩니다. "웹캠에서 사진을 찍는 함수"로만 기억하세요. **핵심은 YOLO 모델 실행 부분(아래쪽)입니다.**
...
if result.boxes:
    print("탐지된 객체가 있습니다.")
    # .cpu() = GPU 메모리에서 CPU로 이동
    # .numpy() = PyTorch 텐서를 numpy 배열로 변환
    # (OpenCV 등 일반 라이브러리는 GPU 텐서를 직접 다룰 수 없기 때문)
...
> **흐름 정리**: CNN 기반 YOLO(속도) → Transformer 기반 DETR(정확도) → 텍스트 기반 Open-Vocabulary(유연성) 순으로 발전하고 있습니다. 실무에서는 목적에 맞는 모델을 선택하는 것이 중요합니다.
...
- [Ultralytics YOLO 공식 문서](https://docs.ultralytics.com/) — 전체 프레임워크 사용법
- [Ultralytics YOLO11 모델 가이드](https://docs.ultralytics.com/models/yolo11/) — 이 수업에서 사용한 모델
- [Ultralytics YOLO26 모델 가이드](https://docs.ultralytics.com/models/yolo26/) — 최신 모델 성능 비교
- [COCO Dataset](https://cocodataset.org/) — 80 클래스 사전학습 데이터셋
- [Roboflow - 커스텀 데이터셋](https://roboflow.com/) — 데이터 라벨링 및 다운로드
- [Pascal VOC Dataset](http://host.robots.ox.ac.uk/pascal/VOC/) — 20 클래스 벤치마크 데이터셋
