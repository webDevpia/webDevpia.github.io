---
title: 3. Agent 및 Task 설계와 프롬프트 작성법
layout: default
grand_parent: LLM
parent: CrewAI
nav_order: 3
permalink: /llm/crewai/agent
--- 
...
# LLM 초기화 (OpenAI GPT-4o-mini 모델 사용)
# 명시적으로 적지 않아도 환경설정일에 OPENAI_API_KEY 가 있으면 openai 패키지가 자동으로 사용됨.
...
itinerary_task = Task(
    description=(
        ...
        "{days}일을 1일차, 2일차와 같이 차수로 나누고, 각 일차로 나누고, 각 일자마다 아침/점심/저녁에 할 활동을 상세히 제안하세요.\n"
...
print("=== [단일 에이전트] 부산 3일 일정 생성 시작 ===")
result_single = crew_single.kickoff(inputs={
    "place": "부산",
    "days": 3
})
print("=== [단일 에이전트] 생성된 부산 3일 일정 ===")
...
print("\n=== [협업 에이전트] 부산 3일 일정 생성 시작 ===")
result_multi = crew_multi.kickoff(inputs={
    "place": "부산",
    "days": 3
})
print("=== [협업 에이전트] 생성된 부산 3일 일정 ===")
...
# 결과가 제대로 안나올경우 최대 15번 반복하도록 기본 셋팅되어 있음.
```

`03-3.travel_planner.py`
...
print("\n=== [협업 에이전트] 개선된 프롬프트로 일정 생성 ===")
result_multi_improved = crew_multi_improved.kickoff(inputs={
    "place": "부산",
    "days": 3,
    "budget_per_day": 20
})
