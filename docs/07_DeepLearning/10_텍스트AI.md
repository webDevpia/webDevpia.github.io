...
[텍스트 AI 기초 - 문자에서 지능으로](https://docs.google.com/presentation/d/18t_hpwhIGcAD49YOj-FUoxjdu-obcJj6PyNioaoDsnA/edit?usp=sharing)
...
지금까지 이미지를 CNN으로 분류하고, YOLO로 탐지했습니다. 이번에는 완전히 다른 종류의 데이터인 **텍스트**를 다룹니다.
...
- 대표적인 토큰화 알고리즘으로 **BPE(Byte Pair Encoding)**가 있으며, GPT 계열이 이를 사용합니다.
...
> **다음 시간 미리보기**: 원-핫 인코딩과 임베딩 개념은 다음 시간(11. 임베딩 모델)에서 PyTorch 코드로 직접 구현합니다.
...
전반부에서는 텍스트를 숫자로 바꾸는 방법(토큰화 → 원-핫 → 임베딩)과, 순서대로 읽는 모델의 발전(RNN → LSTM → GRU)을 다뤘습니다.

후반부에서는 **Attention과 Transformer**, 그리고 현대 LLM(BERT, GPT)으로 넘어갑니다. 순차 처리의 한계를 어떻게 돌파했는지가 핵심입니다.
...
> 당시 소속: Google Brain, Google Research, University of Toronto
...
실제 Transformer는 위의 attention을 **여러 갈래(head)로 동시에** 수행합니다. 이를 **Multi-Head Attention**이라고 합니다.

> 한 사람이 문장을 읽는 것이 아니라, 여러 명이 각자 다른 관점(문법, 의미, 위치 등)에서 동시에 읽고 결과를 합치는 것

이렇게 하면 하나의 attention으로는 잡지 못하는 다양한 관계를 포착할 수 있습니다.

### Positional Encoding: 순서를 알려주는 방법

Self-Attention은 모든 토큰을 한 번에 보기 때문에, 그 자체로는 **단어 순서를 모릅니다**. "나는 학교에 간다"와 "간다 학교에 나는"을 구분할 수 없습니다.

그래서 Transformer는 각 토큰의 위치를 별도의 숫자 패턴(**Positional Encoding**)으로 더해줍니다.

> 마치 영화관 좌석 번호를 입장권에 찍는 것과 같습니다. 표 내용(V)만으로는 어디 앉는지 모르지만, 좌석 번호가 있으면 순서를 알 수 있습니다.
...
**학습 방식**: Causal Language Modeling(인과적 언어 모델링), 즉 자기회귀(next-token prediction)
...
1. 텍스트가 AI에게 어려운 3가지 이유는 무엇인가?
2. 토큰과 단어는 어떻게 다른가?
3. 원-핫 인코딩의 가장 큰 한계 2가지를 말해 보세요.
4. RNN의 기울기 소실 문제를 "전화 게임" 비유로 설명해 보세요.
5. LSTM이 RNN보다 긴 문장을 잘 다루는 이유는 무엇인가?
6. Transformer가 RNN보다 병렬화에 유리한 이유는?
7. BERT와 GPT의 학습 방식 차이를 한 문장으로 쓰세요.
8. GPT, ChatGPT, LLM은 각각 무엇을 의미하는가?

### 심화 과제

- Self-Attention에서 Q, K, V를 검색엔진 비유 외에 다른 비유로 설명해 보기
- "환각(Hallucination)"이 발생하는 근본적인 이유를 이 자료의 내용을 바탕으로 추론해 보기
- 이미지(CNN, YOLO)와 텍스트(RNN, Transformer)에서 AI가 데이터를 다루는 방식의 공통점과 차이점을 정리해 보기
- Positional Encoding이 없으면 Transformer에 어떤 문제가 생길지 구체적 예시를 들어 설명해 보기
