...
- 나만의 이미지를 생성할 수 있고, 배포를 위해서도 많이 할용하는 기능
...
CMD | docker 컨테이너가 시작할 때, 실행하는 쉘 명령을 지정하는 명령  <br> RUN과 비슷하지만 Run은 이미지 작성시 실행하는 명령이고, CMD는 컨테이너를 시작할 때 실행하는 명령임  <br>  (예:CMD ['python','app.py'])
RUN | 쉘 명령을 실행하는 명령  <br>  (예:RUN ['apt-get','install','nginx]),  <br>  RUN은 이미지 작성시 실행되며 일종의 새로운 이미지 layer 를 만드는 역활을 함
ENTRYPOINT | docker 컨테이너가 시작할 때, 실행하는 쉘 명령을 지정하는 명령.  <br> docker run 커맨드 실행시, 별도 명령어도 넣을 수 있는데 이때 CMD 명령은 해당 명령으로 덮어 씌워짐.  <br> ENTRYPOINT로 지정한 명령은 docker run 커멘드 실행 시 함께 넣어진 별도 명령어가 있더라도 덮어씌워지지 않고 실행됨 
EXPOSE | docker 컨테이너 외부에 오픈할 포트 설정 <br> (예:EXPOSE 8080)
ENV | docker 컨테이너 내부에서 사용할 환경 변수 지정 <br> (예 : ENV PATH /usr/bin:$PATH)
WORKDIR | docker 컨테이너에서 작업 디렉토리 설정
COPY | 파일 또는 디렉토리를 docker 컨테이너에 복사.  <br> ADD와 달리 URL은 지정할 수 없으며, 압축 파일을 자동으로 풀어주지 않음 <br> (예:COPY test.sh /root/test.sh)

- 다음 명령도 참고로는 알아두기로 함

| 명령 | 설명 |
| ADD | ADD와 COPY 명령은 유사하지만, COPY 명령이 보다 명시적이므로, COPY 명령을 사용하도록 함 <br> (ADD 명령은 참고로만 알아둠)   <br> 파일,디렉토리, 또는 특정 URL 의 데이터를 docker 이미지에 추가 <br> (예:ADD file /var/www/html)추가할 파일이 tar 압축 파일일 경우 자동으로 압축을 풀어줌, 이 기능이 압축파일을 그대로 넣고 싶을 때가 문제가 되었음) <br> 동일한 이름의 파일 또는 디렉토리가 이미 docker 이미지에 있을 시에는 덮어 씌우지 않음 <br> (예:ADD test.sh /root/test.sh) |
| SHELL | 쉘 프로그램 지정 명령이지만, CMD 등으로 대체 가능 <br> (예:SHELL ['/bin/bash','-c']) |
| ARG | dockerfile 내에서 필요한 변수 설정. <br>  docker 이미지/컨테이터에서 사용하는 환경 변수를 설정하는 ENV와 달리 dockerfile 스크립트 작성을 위해 필요한 변수를 설정 <br> (예:ARG env=dev) |
...
- 기존에 작성한 모든 이미지 및 컨테이너를 삭제하고 시작하자.
...
```bash
# 연습에 필요한 파일 clone 및 경로 이동 후 테스트 진행
git clone https://github.com/shimseonjo1/dockertest.git
cd dockertest
```
...
옵션 | 설명
-t  <br> 또는 <br> --tag | 이미지 이름 설정.  <br> 이미지 이름은 [저장소(DockerHub ID)/이미지이름:태그]와 같이 작성할 수 있음 <br> (저장소 이름 및 태그 이름은 작성안해도 되면, 태그 이름이 없을 경우, 디폴트로 latest)
-f | 이미지 빌드시 디폴트로 Dockerfile 파일명으로 된 파일을 찾아서 이미지를 빌드함.  <br> 그 외의 파일명으로 이미지를 빌드할 경우 해당 옵션을 사용해서 파일명을 지정할 수 있음
--pull | FROM으로 지정된 이미지는 한번 다운로드 받으면 이미지 생성시 마다 새로 다운로드 받지않고 다운로드 받은 이미지를 사용함.  <br> 해당 옵션은 이미지 생성시마다 새로 다운로드를 받으라는 옵션임. <br>  - -pull 와 같이 작성하여 사용. <br> Dockerhub에 베이스 이미지를 수시로 업데이트하고 이를 기반으로 새로운 이미지 생성시 자주 사용할 수 있는 옵션

- 테스트
    - Dockerfile을 작성하고 동일 경로에서 다음과 같이 명령
    - --tag test 는 이미지의 현재 이름을 test로 설정한 것이므로, 디폴트 태그가 붙어서 test:latest로 작성됨
    - 마지막의 "."은 현재 폴더를 나타내는 것이고, 즉 현재 폴더에 Dockerfile파일이 있음을 의미함  <br>  (보다 명시적인 표기를 위해 "./"와 같이 쓰는 것을 추천하기도 함)
...
docker build --tag myimage -f Dockerfile2 .
...
- Dockerfile 생성
    - 하부 폴더 html 폴더를 생성후 이미지에 추가할 파일을 넣어놓은 상태에서 다음과 같이 Dockerfile 생성
...
- 다음 세가지 형태로 CMD 명령을 작성할 수 있음
    - 명령어, 인자값을 리스트처럼 작성하는 형태(해당 방식을 docker에서는 추천)
...
# COPY 가 정상적으로 됐는지 파일 확인 -> 정상적으로 복사됨.
cd /usr/local/apache2/htdocs
ls

# 터미널에서 나가면 컨테이너 실행 중지되고 --rm 옵션으로 run 했으므로 컨테이너 삭제까지 처리됨. 
exit
...
- 위의 이미지 조사에서
    ...
    - 터미널에 접속할 수 있었던 것은 -dit 옵션으로 터미널은 연결된 채로 컨테이너 실행 시 /bin/sh 프로그램이 실행되면서 대기 상태로 되어 있기 때문
...
# /bin/sh /bin/sh -c httpd-foreground 로 실행되므로 오류발생
docker run -dit -p 9999:80 --name httpdweb3 myweb3 /bin/sh -c httpd-foreground

# /bin/sh -c httpd-foreground 로 실행되므로 제대로 실행된다.
docker run -dit -p 9999:80 --name httpdweb3 myweb3 -c httpd-foreground
```
...
```bash
# 이미지 생성
$ docker build --tag myweb3 -f Dockerfile_httpd4 .
...
$ docker build --tag myweb4 -f Dockerfile_httpd5 .
...
```dockerfile
# Dockerfile_httpd6 파일명으로 다음과 같이 작성
...
```bash
$ docker build --tag myweb -f Dockerfile_httpd7 .
```
...
## 2. docker build시 소스파일 github에서 받아오기
...
```bash
docker build --no-cache --tag django -f Dockerfile_github .
docker run -d -p 9999:8000 --name djangotest --rm django
```